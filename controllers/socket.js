...
const useSocket = async(server,sessionID,user) =>{
    ...
    const participants = {patient:session.patient._id,therapist:session.therapist._id};
    ...
    io.on("connection",(socket)=>{
        ...
        if(user.id == patient._id){
            Session.findOneAndUpdate(session,{patient_socket_id:socket.id},{new:true})
            .then((updated)=>{
        ...
        if(session.therapist == user.id){
            Session.findOneAndUpdate(session,{therapist_socket_id:socket.id},{new:true})
            .then(updated =>{
                ...
                socket.to(recieverIDs.patient).emit("server_message",{message:`${socket.userName} is online`,time:Date.now()})
        ...
        socket.on("client_message",(message)=>{
            if(Object.values(recieverIDs).length < 2){
                Session.findOneAndUpdate(session,{"$push":{messages:{userName:socket.userName,message,time:Date.now()}}})
            }
            emmitMessage(socket,{userName:socket.userName,time:Date.now(),message});
        })
        socket.on("disconnect",()=>{
            ...
            Session.findOneAndUpdate(session,{"$push":{messages:socket.messages.sort((a,b)=>{return a.time - b.time })}});
            socket.emit("server_message",{message:`${socket.userName} is offline`,time:Date.now()});
        })
    });
